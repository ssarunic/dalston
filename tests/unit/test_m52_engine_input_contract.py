"""M52 tests for TaskRequest contract hardening."""
...
def test_audio_path_stays_none_without_audio_or_artifacts() -> None:
    input_data = TaskRequest(task_id="task-1", job_id="job-1")
...
def test_typed_output_raises_on_invalid_structure() -> None:
    input_data = TaskRequest(
        task_id="task-1",
        job_id="job-1",
        previous_responses={"transcribe": {"segments": "invalid-shape"}},
    )
...
def test_typed_output_returns_none_when_key_missing() -> None:
    input_data = TaskRequest(task_id="task-1", job_id="job-1", previous_responses={})
