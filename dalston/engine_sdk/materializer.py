...
class S3ArtifactStore(ArtifactStore):
    """Artifact transport using presigned URLs via HTTP (M77).

    Downloads use the presigned GET URL embedded in each ArtifactReference.
    Uploads use presigned PUT URLs pre-generated by the orchestrator at dispatch time
    and keyed by storage locator (s3:// URI).
    """

    def __init__(self, upload_url_map: dict[str, str] | None = None) -> None:
        self._upload_url_map: dict[str, str] = upload_url_map or {}

    def download(self, ref: ArtifactReference, destination: Path) -> None:
        if not ref.download_url:
            raise RuntimeError(
                f"ArtifactReference '{ref.artifact_id}' has no download_url. "
                "Ensure the orchestrator is M77-compatible and generates presigned GET URLs."
            )
        http.stream_to_file(ref.download_url, destination)

    def upload(self, source: Path, locator: str) -> None:
        upload_url = self._upload_url_map.get(locator)
        if not upload_url:
            raise RuntimeError(
                f"No presigned upload URL for locator: {locator}. "
                "Ensure the orchestrator pre-generates PUT URLs for all produced artifacts."
            )
        content_type = _guess_content_type(source)
        http.stream_from_file(upload_url, source, content_type=content_type)
...
class LocalFilesystemArtifactStore(ArtifactStore):
    ...
    def download(self, ref: ArtifactReference, destination: Path) -> None:
        source = Path(ref.storage_locator)
    ...
    def materialize(
        ...
        *,
        ...
        for slot, artifact_id in resolved_artifact_ids.items():
            ...
            self.store.download(ref, destination)
