...
class TaskInputData(BaseModel):
    ...
    output_url: str = Field(
        description="Presigned PUT URL for engine to store output.json (M77)",
    )
    artifact_upload_urls: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Presigned PUT URLs for produced artifact files, keyed by storage locator "
            "(s3:// URI). Pre-generated by orchestrator at dispatch time (M77)."
        ),
    )
...
class PrepareOutput(BaseModel):
    ...
    engine_id: str = Field(..., description="Runtime identifier")
...
class Transcript(BaseModel):
    """Canonical transcript output — returned by every transcription engine_id.
    ...
    engine_id: str = Field(
        ..., description="Runtime identifier (e.g., 'faster-whisper')"
    )
    channel: int | None = Field(
        default=None,
