...
- Multi-stage build keeps the engine_id image smaller by excluding build tools
...
9. Add Docker Compose service definition for `stt-transcribe-parakeet`
...
4. **Multi-stage Docker build** — NeMo's build dependencies are heavy. The multi-stage build reduces the engine_id image size while keeping build-time tools in the builder stage.
