...
from dalston_cli.commands import transcribe, listen, jobs, export, status, models
...
@click.option('--pii-tier', type=click.Choice(['fast', 'standard', 'thorough']),
              help='PII detection tier')
@click.option('--pii-entities', help='Comma-separated entity types to detect')
@click.option('--redact-audio/--no-redact-audio', default=False,
              help='Generate redacted audio file')
@click.option('--redaction-mode', type=click.Choice(['silence', 'beep']),
              help='Audio redaction mode')
@click.pass_context
def transcribe(ctx, files, language, output, fmt, wait, json_output,
               speakers, num_speakers, timestamps, no_speakers,
               pii, pii_tier, pii_entities, redact_audio, redaction_mode):
    ...
    for file_path in files:
        ...
        job = client.transcribe(
            ...
            pii_entity_types=pii_entities.split(',') if pii_entities else None,
            redact_pii_audio=redact_audio,
            pii_redaction_mode=redaction_mode,
        )
...
```python
# dalston_cli/commands/models.py
import click
import json
...
@click.pass_context
def models(ctx, model_id, as_json):
    ...
    if model_id:
        ...
        if as_json:
            ...
            click.echo(f"{model.name} ({model.id})")
            click.echo(f"  {model.description}")
            click.echo(f"  Tier: {model.tier}")
            click.echo(f"  Languages: {model.capabilities.languages}")
            click.echo(f"  Word timestamps: {model.capabilities.word_timestamps}")
            click.echo(f"  Streaming: {model.capabilities.streaming}")
    else:
        # List all models
        ...
        if as_json:
            ...
            for model in model_list.models:
                lang_info = "multilingual" if model.capabilities.languages > 1 else "English only"
                click.echo(f"  {model.id}")
                click.echo(f"    {model.description}")
                click.echo(f"    Tier: {model.tier} | Languages: {lang_info}")
                click.echo()
...
- `dalston transcribe` - Batch transcription with all options (language, format, speakers, timestamps)
- `dalston listen` - Real-time microphone capture with live/json/jsonl output modes
- `dalston jobs list|get|wait` - Job management commands
- `dalston export` - Export transcripts to txt/json/srt/vtt
- `dalston status` - Server health and real-time capacity display
- `dalston models` - List available models and their capabilities
- Global options: `--server`, `--api-key`, `--verbose`, `--quiet`
- Config file support (`~/.dalston/config.yaml`)
- Environment variable support (`DALSTON_SERVER`, `DALSTON_API_KEY`)

### SDK Changes

The SDK Python package was renamed from `dalston` to `dalston_sdk` to avoid import conflicts with the `dalston/` directory in the repo root (server code). The PyPI distribution name remains `dalston-sdk`.

Added two new methods to support the CLI:

- `Dalston.health()` - Check server health via GET /health
- `Dalston.get_realtime_status()` - Get real-time capacity via GET /v1/realtime/status

### Deferred

- Shell completion (bash/zsh/fish) - Future enhancement
- Interactive TUI mode - Future enhancement
- Watch mode for directory monitoring - Future enhancement

### PII Detection (M26)

Added PII detection options to `dalston transcribe`:

- `--pii/--no-pii` — Enable PII detection in transcript
- `--pii-tier` — Detection tier (fast, standard, thorough)
- `--pii-entities` — Comma-separated entity types to detect
- `--redact-audio/--no-redact-audio` — Generate redacted audio file
- `--redaction-mode` — Audio redaction mode (silence, beep)

Example:

```bash
dalston transcribe call.mp3 --pii --pii-tier standard --redact-audio --redaction-mode beep
```

### Sessions Commands (M24)

Added realtime session management via `dalston sessions`:

- `dalston sessions list` — List realtime sessions with status filter and JSON output
- `dalston sessions get <id>` — Get detailed session information
- `dalston sessions delete <id>` — Delete a non-active session (with confirmation prompt)

Options:

- `--status` — Filter by session status (active, completed, error, interrupted)
- `--limit` — Maximum sessions to return (default 20)
- `--json` — Output as JSON
- `--force` — Skip delete confirmation prompt
