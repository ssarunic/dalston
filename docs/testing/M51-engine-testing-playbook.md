...
| `pii_detect` | Primarily `previous_responses` (transcribe/align/diarize) | `PIIDetectOutput` typed payload | No hidden dependency on audio URIs |
...
6. Tests still import deprecated `TaskInput` / `TaskOutput` aliases instead of `EngineRequest` / `EngineResponse`.
