...
for i in {1..30}; do
  if ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
       ubuntu@$INSTANCE_TAG "docker ps --format '{{.Names}}' 2>/dev/null | grep -q $CONTAINER_NAME" 2>/dev/null; then
...
ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no ubuntu@$INSTANCE_TAG "docker logs -f $CONTAINER_NAME"
